...
TACTIC "copy introforward" (rule) IS
    WHEN    (LETHYP _A (ForwardCut 0 (LAYOUT "copy" (0) (WITHARGSEL rule))))
            (ComplainForwardNoHyp "copy" "" "")

TACTIC "LEM inventforward" (rule) IS
    WHEN    (LETHYP _A (ForwardCut 0 (LAYOUT "LEM invent" (0) (WITHARGSEL rule))))
            (ComplainForwardNoHyp "LEM invent" "" "")
...
/*** Forward derived rule: Modus Tollens ***/
TACTIC "→ MT forward" IS
    WHEN    
        (LETHYP2 _C (_A→_B) 
            (Noarg (CUTIN "→ MT" (WITHHYPSEL (hyp (_A→_B))) (WITHHYPSEL (hyp ¬_B))) "→ MT"))
        (LETHYP2 _C (_A→_B)
            ("→ MT forward fail"
                (" You selected %t, which is of the form A→B, but your other selection %t doesn't match %t.", _A→_B, _C, _A)))
        (LETHYP2 _A _B
            ("→ MT forward fail" (" Neither of your selections (%t and %t) is of the form A→B.", _A, _B)))
        (LETHYP (_A→_B) 
            (WHEN (LETCONC ¬_A 
                    (Noarg (CUTIN "→ MT" (WITHHYPSEL hyp) fstep) "→ MT"))
                    ("→ MT forward fail" (" You only selected %t.", _A→_B))))
        (LETHYP _A
            ("→ MT forward fail" (" You selected %t, which isn't of the form A→B.", _A)))
        (LETHYPS _A
            ("→ MT forward fail" (" You selected too many hypotheses – %l.", (_A, ", ", " and "))))
        ("→ MT forward fail" (" You didn't select anything."))
                
TACTIC "→ MT forward fail" (extra) IS
        Fail    ("To make a forward step with → elim MT you must select something of the form A→B, \
            \and something which matches B (or a target conclusion).%s", extra)
...
MENU Forward IS
    ...
    ENTRY   "∨ elim (makes assumptions)"    IS Forward (QUOTE (_A∨_B)) 
                                                        ...
                                                        "∨ elim" "∨ intro" "A∨B"
    ENTRY   "¬ elim"                        IS "¬ elim forward"
    ENTRY   "¬¬ elim" IS Forward (QUOTE (¬¬_A)) (ForwardCut 0 (Noarg (LAYOUT "¬¬ elim" (0) "¬¬ elim") "¬¬ elim"))   "_A"
...
/*** DERIVED RULES: Useful in verifying course material ***/    
    SEPARATOR
    ENTRY   "→ elim MT"                    IS "→ MT forward"

    ENTRY   "¬¬ intro"                      IS Forward (QUOTE _A) (ForwardCut 0 (Noarg (LAYOUT "¬¬ intro" (0) "¬¬ intro") "¬¬ intro"))   "¬¬_A"
    ENTRY   "LEM invent"                     IS "LEM inventforward" "LEM invent"
    
    ENTRY   "copy select"                     IS "copy introforward" "copy(R)"
